...
articlesRouter
  ...
  .post(jsonParser, (req, res, next) => {
    ...
    ArticlesService.insertArticle(req.app.get('db'), newArticle)
      .then(article => {
        res.status(201).location(path.posix.join(req.originalUrl + `/${article.id}`)).json(article);
      })
      .catch(next);
  });
...
articlesRouter
  ...
  .get((req, res, next) => {
    res.json({
      ...
      title: res.article.title, // sanitize title
      content: res.article.content, // sanitize content
